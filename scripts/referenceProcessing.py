...
def histogramTechnicalRep(snpProportion, sampleMeta):
    ...
    for i in repNames[repCounts > 1]:
        ...
        subset = snpProportion[
            snpProportion.columns[np.isin(snpProportion.columns, shortName)]
        ].values

        for j in list(
            itertools.combinations(np.arange(len(shortName)), 2)
        ):  # if there are more than two technical rep, do all pairwise combinations
...
def splitReferences(snpProportion, sampleMeta, communities):
    ...
    """
    logger.debug("Varieties that are split across multiple clusters")
    ...
    for ref in np.unique(w["reference_original"]):
        shortNames = w[w["reference_original"] == ref]["short_name"].values.astype(
            "str"
        )
        if (
            len(
                np.unique(
                    communities[np.where(np.isin(snpProportion.columns, shortNames))[0]]
                )
            )
            > 1
        ):
...
def referenceDistance(snpProportion, sampleMeta):
    ...
    for ref in np.unique(w["reference"]):
        ...
        if len(shortNames) > 1:
            maxDistance.append(
                sch.linkage(snpProportion[shortNames].values.T, metric="correlation")[
                    -1, 2
                ]
            )
    ...
    plt.xticks(
        len(maxDistance) * np.arange(0, 1.1, 0.1), np.around(np.arange(0, 1.1, 0.1), 1)
    )
    plt.tight_layout()
