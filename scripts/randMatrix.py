...
def calc_cluster_characteristics(k, data, clusteringMethod, **kwargs):
    ...
    logger.debug("Parameter value:", k)
    ...
    if clusteringMethod == "DBSCAN":
        ...
        results["communities"], _ = labelHCLandrace(
            data, sampleMeta, Y_cluster, cutHeight=k, newVariety=False
        )

    else:
        logger.error("invalid clustering method")
...
def randScoreMatrix(data, ks, clusteringMethod, **kwargs):
    ...
    if sampleMeta is None:
        cluster_chars_list = [
            calc_cluster_characteristics(
                k, data, clusteringMethod, admixedCutoff=admixedCutoff
            )
            for k in ks
        ]
        ...
        cluster_chars_list = [
            calc_cluster_characteristics(
                k, data, clusteringMethod, sampleMeta=sampleMeta
            )
            for k in ks
        ]
...
def labelHCLandrace(
    ...
    for cluster in np.unique(subClusters):
        ...
        shortName = clusterSubset.columns[sampleIndex][
            np.isin(
                clusterSubset.columns[sampleIndex],
                references["short_name"].values.astype("str"),
            )
        ]
        refInSubCluster = np.unique(
            sampleMeta[sampleMeta["short_name"].isin(shortName.astype("int"))][
                "reference"
            ]
        )
        ...
        if len(refInSubCluster) > 0:  # if there's a reference in the cluster
            ...
            if not np.isin(
                refInSubCluster, subClusterNames
            ):  # add if it's a new reference
                subClusterNames.append(list(refInSubCluster))

            subClusterNumber[sampleIndex.tolist()] = np.where(
                subClusterNames == refInSubCluster
            )[0][0]

        elif newVariety:  # add new non reference varieties
            if not admixedCutoff:  # no admixedCutoff
                subClusterNames.append(
                    ["Genetic entity-" + str(clusterNumber) + "-" + str(counter)]
                )
                subClusterNumber[sampleIndex.tolist()] = len(subClusterNames) - 1
                counter += 1

            elif (
                min(plot.homozygousDivergence(clusterSubset.values[:, sampleIndex]))
                < admixedCutoff
            ):  # at least one sample must be less than admixedCutoff
                subClusterNames.append(
                    ["Genetic entity-" + str(clusterNumber) + "-" + str(counter)]
                )
                subClusterNumber[sampleIndex.tolist()] = len(subClusterNames) - 1
...
def cutoffQuality(clusterSubset, sampleMeta, Y_cluster):
    ...
    """
    references = sampleMeta[(sampleMeta["reference"].notna())]
    refRows = np.arange(len(clusterSubset.columns))[
        np.isin(clusterSubset.columns, references["short_name"].values.astype("str"))
    ]

    varNames = []
    for i in clusterSubset.columns[refRows]:
        varNames.append(
            references[references["short_name"] == int(i)]["reference"].values[0]
        )
    ...
    repTogether = np.zeros(
        len(cutList)
    )  # number of technical replicates in same cluster
    test = pd.DataFrame(
        columns=df.columns, index=np.arange(len(np.unique(varNames)))
    )  # number of clusters that contain more than one variety

    for i, var in enumerate(np.unique(varNames)):
        rows = refRows[np.where(varNames == var)]
        repTogether += (
            pd.Series({c: len(df.iloc[rows][c].unique()) for c in df}).values == 1
        )
        test.iloc[i] = pd.Series(
            df.iloc[rows][c].unique() for c in df.columns
        )  # don't count the same variety multiple times
    ...
    plt.figure()
    plt.plot(
        cutList, repTogether, "-o", markersize=2, label="Technical replicates together"
    )
    plt.plot(
        cutList, avgVarInCluster, "-^", markersize=2, label="Avg varieties/cluster"
    )
