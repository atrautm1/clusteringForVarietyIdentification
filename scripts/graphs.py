...
def umapCluster(embedding, communities):
    ...
    for i in np.unique(communities):
        ...
        ax1.text(
            np.mean(embedding[samples, 0]),
            np.mean(embedding[samples, 1]),
            i,
            fontsize=12,
        )
    ...
    cbar.set_ticks(
        (np.linspace(0, max(communities), max(communities) + 2)[:-1] + boxWidth / 2),
        labels=np.arange(max(communities) + 1).astype("str"),
    )
    plt.tight_layout()
...
def umapReference(snpProportion, embedding, sampleMeta, communities):
    ...
    referencesIndex = np.where(
        np.isin(snpProportion.columns, references["short_name"].astype("str"))
    )[0]
    ...
    for i, j in enumerate(
        refClusters
    ):  # all the ref in a cluster should be the same color
    ...
    ax2.set_yticks(
        np.arange(len(labels)) + 0.5, labels, fontsize=5
    )  # need to offset labels by box width
...
def heatmapSingleCluster(snpProportion, sampleMeta, communities, COI):
    ...
    subset = snpProportion[
        snpProportion.columns[np.where(communities == COI)[0]]
    ].values
    ...
    numInRef, numInCluster, numInZ = np.intersect1d(
        referencesIndex, np.where(communities == COI)[0], return_indices=True
    ...
    if len(numInRef) > 0:
        ...
        for i in range(len(numInRef)):
            ...
            labels.append(
                sampleMeta[
                    sampleMeta["short_name"] == int(snpProportion.columns[numInRef[i]])
                ]["reference"].values[0]
            )
...
def heatmapManyClusters(
    snpProportion, sampleMeta, communities, allCOI, tickType="blank"
):
    """
    ...
    subsetAllCOI = snpProportion[
        snpProportion.columns[np.where(np.isin(communities, allCOI))]
    ]
    ...
    gs = GridSpec(
        1, len(allCOI) + 1, width_ratios=[1] * len(allCOI) + [0.05], figure=fig
    )
    ...
    for index in range(len(allCOI)):
        ...
        if (
            tickType == "references"
        ):  # add reference labels to x-ticks (one reference per variety)
            numInRef, numInCluster, numInZ = np.intersect1d(
                referencesIndex, clusterIndex, return_indices=True
            )  # number of the reference
            if len(numInRef) > 0:
                ticks = []
                labels = []
                for i in range(len(numInRef)):
                    ...
                    labels.append(
                        sampleMeta[
                            sampleMeta["short_name"]
                            == int(snpProportion.columns[numInRef[i]])
                        ]["reference"].values[0]
                    )

                uniqueLabel, uniqueIndex = np.unique(labels, return_index=True)
                subplot.set_xticks(
                    np.asarray(ticks)[uniqueIndex],
                    np.asarray(labels)[uniqueIndex],
                    rotation=90,
                )
            else:
                subplot.set_xticks([])
        ...
        ):  # add reference labels to x-ticks (all references)
            numInRef, numInCluster, numInZ = np.intersect1d(
                referencesIndex, clusterIndex, return_indices=True
            )  # number of the reference
            if len(numInRef) > 0:
        ...
        if tickType == "sampleRef":  # add sample names and reference labels to x-ticks
            ...
            if len(numInRef) > 0:
                for i in range(len(numInRef)):
                    labels[np.where(Z_cluster == numInZ[i])[0][0]] = sampleMeta[
                        sampleMeta["short_name"]
                        == int(snpProportion.columns[numInRef[i]])
                    ]["reference"].values[0]
        ...
        if tickType == "sampleNames":  # add sample names to x-ticks
            subplot.set_xticks(
                np.arange(len(clusterIndex)),
                snpProportion.columns[clusterIndex[Z_cluster]],
                rotation=90,
            )
        ...
        if tickType == "divergence":  # add sample divergence to x-ticks
            ...
            subplot.set_xticks(
                np.arange(len(divergence)), np.around(divergence, 2), rotation=90
            )
        ...
        if tickType == "sampleDivergence":  # add sample names and divergence to x-ticks
            ...
            for i in range(len(sampleName)):
                labels.append(sampleName[i] + "-" + str(divergence[i]))
...
def heatmapReferences(snpProportion, sampleMeta, allVarieties, tick_type):
    ...
    refShort = sampleMeta[
        (
            np.isin(sampleMeta["reference_original"], allVarieties)
            if "reference_original" in sampleMeta
            else np.array([False])
        )
    ...
    if tick_type == "inventory":  # add inventory number to x-ticks
        for sample in sampleOrder:
            labels.append(
                sampleMeta[sampleMeta["short_name"] == int(sample)]["inventory"].values[
                    0
                ]
            )
    ...
    if tick_type == "source":  # add sample source to x-ticks
        for sample in sampleOrder:
            ...
            labels.append(
                row["reference"].values[0] + ", " + row["seedSource"].values[0]
            )

    if tick_type == "references":  # add reference labels to x-ticks (all references)
        for sample in sampleOrder:
            labels.append(
                sampleMeta[sampleMeta["short_name"] == int(sample)]["reference"].values[
                    0
                ]
            )
...
def histogramMissingness(snpProportionNoInterpolation):
    ...
    ax1, ax2 = plotDouble()
    ax1.hist(
        snpProportionNoInterpolation.isna().sum(axis=0),
        bins=np.linspace(0, snpProportionNoInterpolation.shape[0], 21),
    )  # samples missing values from X genes
    ax1.set_xticks(
        np.linspace(0, snpProportionNoInterpolation.shape[0], 21), np.arange(0, 105, 5)
    )
    ax1.set_title("Missingness per sample")
    ax1.set_xlabel("Percent Missingness")
    ax2.hist(
        snpProportionNoInterpolation.isna().sum(axis=1),
        bins=np.linspace(0, snpProportionNoInterpolation.shape[1], 21),
    )  # gene missing from X samples
    ax2.set_xticks(
        np.linspace(0, snpProportionNoInterpolation.shape[1], 21), np.arange(0, 105, 5)
    )
...
def histogramAverageCounts(countsFile, snpProportion, embedding):
    ...
    counts = counts[snpProportion.columns].loc[
        snpProportion.index
    ]  # subset to the samples, loci in snpProportion
    ...
    im = ax2.scatter(
        embedding[belowcut, 0], embedding[belowcut, 1], s=1, c=avgCounts[belowcut]
    )
    plt.colorbar(im)
    plt.tight_layout()
...
def barchartRef(snpProportion, output, sampleMeta):
    ...
    refshort = w["short_name"][(sampleMeta["reference"].notna())].values.astype(
        "str"
    )  # references
    admixedshort = output["short_name"][output["variety"] == "Admixed"].values.astype(
        "str"
    )  # admixed

    landraceName = var[
        np.flatnonzero(
            np.core.defchararray.find(var.astype("str"), "Genetic entity") != -1
        )
    ]
    landraceShort = output["short_name"][
        np.isin(output["variety"], landraceName)
    ].values.astype("str")

    callshort = np.setdiff1d(
        output["short_name"], np.concatenate((refshort, admixedshort, landraceShort))
    )
    callVarieties, callVarietiesCount = np.unique(
        output[output["short_name"].isin(callshort)]["variety"], return_counts=True
    )

    fig, ax = plt.subplots(figsize=(14.4, 4.8))
    ax.barh(
        np.arange(len(callVarieties)),
        callVarietiesCount[np.argsort(callVarietiesCount)],
    )
    ax.bar_label(ax.containers[0], label_type="edge")
    ax.set_yticks(
        np.arange(len(callVarieties)), callVarieties[np.argsort(callVarietiesCount)]
    )
    plt.tight_layout()
...
def umapRefLandrace(
    ...
    cutoff,
    noRef=False,
    fig_title="UMAP clustering predictions",
):
    """
    ...
    landraceLowName = var[counts < cutoff][
        np.flatnonzero(
            np.core.defchararray.find(
                var[counts < cutoff].astype("str"), "Genetic entity"
            )
            != -1
        )
    ]
    landraceLowshort = output["short_name"][
        np.isin(output["variety"], landraceLowName)
    ].values.astype(
        "str"
    )  # landrace < cutoff
    landraceHighName = var[counts >= cutoff][
        np.flatnonzero(
            np.core.defchararray.find(
                var[counts >= cutoff].astype("str"), "Genetic entity"
            )
    ...
    landraceHighshort = output["short_name"][
        np.isin(output["variety"], landraceHighName)
    ].values.astype(
        "str"
    )  # landrace >= cutoff
    landraceHighVarieties = np.unique(
        output[output["short_name"].isin(landraceHighshort)]["variety"]
    )

    callshort = np.setdiff1d(
        output["short_name"],
        np.concatenate((refshort, admixedshort, landraceLowshort, landraceHighshort)),
    )
    callVarieties = np.unique(output[output["short_name"].isin(callshort)]["variety"])

    ax1, ax2, fig = plotDouble()
    ...
    if noRef == True:
        ...
        ax1.scatter(
            output["embedding_X"][noRefCol].values,
            output["embedding_Y"][noRefCol].values,
            s=1,
            c="grey",
            alpha=0.1,
        )
        ...
        ax1.scatter(
            output["embedding_X"].values,
            output["embedding_Y"].values,
            s=1,
            c="grey",
            alpha=0.1,
        )
    for i, varietyName in enumerate(callVarieties):
        sampleNames = np.setdiff1d(
            output["short_name"][output["variety"] == varietyName].values.astype("str"),
            refshort,
        )
        if len(sampleNames) > 0:
            sampleIndex = np.where(np.isin(snpProportion.columns, sampleNames))[0]
            ax1.scatter(
                output.iloc[sampleIndex]["embedding_X"].values,
                output.iloc[sampleIndex]["embedding_Y"].values,
                s=2,
                color=tab20(i),
                label=varietyName,
            )
...
def umapReferenceSeparate(snpProportion, embedding, sampleMeta):
    ...
    for i in range(0, midPoint):
        ...
        referencesIndex = np.where(
            np.isin(snpProportion.columns, rows["short_name"].astype("str"))
        )[0]
        ax1.scatter(
            embedding[referencesIndex, 0],
            embedding[referencesIndex, 1],
            s=3,
            color=tab20(i),
            label=refNames[i],
        )
...
def umapDivergence(snpProportion, embedding):
    ...
    ax1.scatter(embedding[:, 0], embedding[:, 1], s=1, c="grey", alpha=0.1)
    SC = ax1.scatter(
        embedding[abovecut, 0], embedding[abovecut, 1], s=1, c=divergence[abovecut]
    )
    plt.colorbar(SC, cax=ax2)
    plt.tight_layout()
...
def histogramDivergence(snpProportion, sampleMeta):
    ...
    ax1, ax2 = plotDouble()
    references = sampleMeta[(sampleMeta["reference"].notna())]
    sampleIndex = np.where(
        ~np.isin(snpProportion.columns, references["short_name"].astype("str"))
    )[0]
    divergenceBySample = homozygousDivergence(
        snpProportion[snpProportion.columns[sampleIndex]]
    )
    ...
    divergenceByRef = homozygousDivergence(
        snpProportion[snpProportion.columns[referencesIndex]]
    )
...
def dendrogram(
    ...
    Y_cluster = sch.linkage(
        clusterSubset.values.T, metric="correlation"
    )  # sort samples

    if tick_type == "sampleRef":  # add sample number and reference to x-ticks
        references = sampleMeta[(sampleMeta["reference"].notna())]
        refInCluster = clusterSubset.columns[
            np.isin(
                clusterSubset.columns, references["short_name"].values.astype("str")
            )
        ]
        labels = np.copy(clusterSubset.columns.values)
        for i in refInCluster:
            index = np.where(clusterSubset.columns == i)[0][0]
            ref = (sampleMeta[sampleMeta["short_name"] == int(i)]["reference"]).values[
                0
            ]
            labels[index] = ref
...
def umapReleaseYear(snpProportion, sampleMeta, embedding):
    ...
    released = sampleMeta[
        (sampleMeta["release_year"].notna()) & (sampleMeta["release_year"] > 0)
    ]
    releasedIndex = np.where(
        np.isin(snpProportion.columns, released["short_name"].astype("str"))
    )[0]

    year = []
    for sample in snpProportion.columns[
        releasedIndex
    ]:  # these need to be in the same order
        year.append(
            released[released["short_name"] == int(sample)]["release_year"].values[0]
        )

    ax1, ax2 = plotTemplate()
    ax1.scatter(embedding[:, 0], embedding[:, 1], s=1, c="grey", alpha=0.1)
    SC = ax1.scatter(
        embedding[releasedIndex, 0], embedding[releasedIndex, 1], s=1, c=year
    )
    plt.colorbar(SC, cax=ax2)
    plt.tight_layout()
...
def umapRefCalls(snpProportion, output, sampleMeta, noRef=False, minYear=None):
    ...
    callshort = np.setdiff1d(
        output["short_name"], np.concatenate((admixedshort, landraceshort, refshort))
    )
    callVarieties = np.unique(output[output["short_name"].isin(callshort)]["variety"])
...
def heatmapDendrogramAll(
    ...
    for clusterNum in np.unique(communities):
        heatmapManyClusters(
            snpProportion, sampleMeta, communities, [clusterNum], tickType=heatmapTick
        )
        plt.savefig(
            filePrefix + " heatmap cluster " + str(clusterNum) + ".png", dpi=300
        )
        plt.close()
        dendrogram(
            snpProportion,
...
def barchartLandrace(snpProportion, output, sampleMeta, filePrefix=None, cutoff=2):
    ...
    landraceVarieties, landraceVarietiesCount = np.unique(
        output[output["short_name"].isin(landraceShort)]["variety"], return_counts=True
    )

    landraceVarietiesFilter = landraceVarieties[landraceVarietiesCount > cutoff]
    landraceVarietiesCountFilter = landraceVarietiesCount[
        landraceVarietiesCount > cutoff
    ]

    fig, ax = plt.subplots(figsize=(14.4, 4.8))
    ax.barh(
        np.arange(len(landraceVarietiesFilter)),
        landraceVarietiesCountFilter[np.argsort(landraceVarietiesCountFilter)],
    )
    ax.bar_label(ax.containers[0], label_type="edge")
    ax.set_yticks(
        np.arange(len(landraceVarietiesFilter)),
        landraceVarietiesFilter[np.argsort(landraceVarietiesCountFilter)],
    )
    plt.tight_layout()
    if filePrefix:
    ...
    if landraceVarietiesCount.size > 0:
        plt.hist(
            landraceVarietiesCount, bins=np.arange(max(landraceVarietiesCount) + 2)
        )
...
def heatmapDendrogram(
    snpProportion, sampleMeta, communities, COI, cutHeight, cutLine=False
):
    """
